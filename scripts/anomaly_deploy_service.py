...
class AnomalyDeployService():
    ...
    def _navigate_to(self, waypoint_hash):
        """Navigate to a specific waypoint."""
        self._lease = self._lease_wallet.get_lease()
        destination_waypoint = graph_nav_util.find_unique_waypoint_id(
            waypoint_hash, self._current_graph, self._current_annotation_name_to_wp_id)
        if not destination_waypoint:
    ...
    def run(self, destination_waypoint_id):
        print("Before upload")
        # Upload graph and snapshots
